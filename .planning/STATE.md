...
last_activity: "2026-05-25 — Completed quick task 260525-eva: fix Photos tile in the 2x2 action grid under the 3D Virtual Tour card on PropertyDetailsScreen. The tile was incorrectly setting photoTarget = media.photos[0] and pushing a regular hero JPG into the Tour3DScreen WebView via setActivePhotosUrl. Investigation surfaced a schema gap: the M3 nested Property type intentionally has NO tour-photos URL field (legacy panoramicPhotosUrl was removed at M3 Phase 2 D-20 cutover), and adding one is milestone-shaped (backend Mongoose + nested type + ContextualListingFlow capture + admin MediaCurationScreen capture + migration). Quick-task disposition: added one thin reader src/utils/getTourPhotosUrl.ts returning undefined today + co-located test pinning the current behavior across 4 property shapes; wired the Photos tile IIFE through getTourPhotosUrl(property) (replacing media.photos[0]) so the tile lands in its existing disabled-state path (mirrors the Instagram tile pattern: opacity 0.6, colors.textSecondary/textTertiary, onPress=undefined, disabled=true). Block comment at PropertyDetailsScreen.tsx:942–949 rewritten to reference 260525-eva and the new reader. When the schema decision lands in a future milestone, only getTourPhotosUrl.ts needs to change — the tile, setActivePhotosUrl overlay wiring, and Tour3D WebView all auto-light. No new i18n keys (tile keeps t('property.photos') in EN+RU), no theme-token changes, no Property-type schema change, no react-navigation or Firebase SDK touched. Code: 2 atomic commits (d34c821 reader + f8bac08 tile gate), worktree merged to main (381ff93). Verification: 4/4 new tests pass; 6/6 across PropertyDetailsScreen|getTourPhotosUrl; npx tsc --noEmit adds 0 new errors (same baseline as 260515-iqi). Pending USER manual physical-device QA (non-blocking) — confirm top carousel still swipes, 3D Tour hero still opens, Photos tile grayed at 0.6 opacity in dark+light, tapping tile is a no-op."
