...
| A8 | Library's `KeyboardAvoidingView` defaults to a behavior that works on both platforms with no `behavior` prop — i.e. omitting `behavior` is the idiomatic usage. | §3.5 | **DISPROVEN 2026-04-23 (Plan 02-06 matrix walk).** Library TypeScript defs (`node_modules/react-native-keyboard-controller/lib/typescript/components/KeyboardAvoidingView/index.d.ts:29`) declare `behavior?: "height" \| "padding" \| "position" \| "translate-with-padding"` — optional with NO default. When omitted, the component no-ops; composer stays blocked by keyboard on both iOS and Android. Resolution: both chat screens now pass `behavior="padding"` explicitly per the library's official example code. Fix committed as gap-closure on top of Plan 02-05. The doc guidance "library defaults handle behavior cross-platform" under `<must_haves>` in 02-05-PLAN.md is INCORRECT and should not be copied forward to future work. |
