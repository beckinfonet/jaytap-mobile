...
|-------|--------|-----------------|-----|---------|-------|
| 1 | LoginScreen | email TextInput, PasswordTextInput (eye icon toggle under keyboard) | ⬜ | ⬜ | TBD |
| 2 | SignupScreen | email TextInput, password PasswordTextInput, confirm PasswordTextInput | ⬜ | ⬜ | TBD |
| 3 | ForgotPasswordScreen | email TextInput (autoFocus initial-pop case per PATTERNS line 424) | ⬜ | ⬜ | TBD |
| 4 | ResetPasswordScreen | linkOrCode TextInput (multiline — L10), new password, confirm password | ⬜ | ⬜ | TBD |
| 5 | CreateListingScreen — admin-verify branch | no inputs today — verify scroll & 3× Switch row interaction + save button remain unbroken | ⬜ | ⬜ | TBD |
| 6 | CreateListingScreen — main form | sample 3 inputs spanning top / middle / bottom (title, address, description) | ⬜ | ⬜ | TBD |
| 7 | ChatThreadScreen | composer TextInput (multiline — L10) + Android hardware back dismisses keyboard without popping screen | ⬜ | ⬜ | TBD |
...
|-------|--------|-------|-----|---------|-------|
| 9 | AccountSettingsScreen | one TextInput while in edit mode (tap e.g. Name row) | ⬜ | ⬜ | TBD |
| 10 | HomeScreen | search TextInput at top of home tab | ⬜ | ⬜ | TBD |
...
|------|-----|---------|-------|
| ChatThread composer grows multiline | ⬜ | ⬜ | TBD |
| ChatCompose composer grows multiline | ⬜ | ⬜ | TBD |
| ResetPassword linkOrCode grows multiline | ⬜ | ⬜ | TBD |
...
| Field | Value |
|-------|-------|
| DECISION | `{pending — awaiting Tier 2 row 10 result}` |
| RATIONALE | `{pending}` |
...
|------|---------|----------|--------|
| KBD-02 / SC3.a — zero magic offsets in src/ | `grep -rn "keyboardVerticalOffset" src/ \| wc -l` | 0 | ⬜ |
...
| SC1.c — Babel plugin is LAST | `tail -n 5 babel.config.js \| grep -c "react-native-worklets/plugin"` | 1 | ⬜ |
| SC2.a — keyboard-controller installed | `grep -c "react-native-keyboard-controller" package.json` | 1 | ⬜ |
| SC2.b — KeyboardProvider wired in App.tsx | `grep -cE "KeyboardProvider" App.tsx` | ≥ 2 | ⬜ |
| KBD-02 — chat surface clean of magic offsets | `grep -rn "keyboardVerticalOffset" src/screens/Chat*Screen.tsx \| wc -l` | 0 | ⬜ |
| KBD-01 (auth) — 4 auth screens KASV-wrapped | `grep -l "KeyboardAwareScrollView" src/screens/{Login,Signup,ForgotPassword,ResetPassword}Screen.tsx \| wc -l` | 4 | ⬜ |
| KBD-01 (listing) — CreateListing both branches | `grep -c "KeyboardAwareScrollView" src/screens/CreateListingScreen.tsx` | ≥ 2 | ⬜ |
| KBD-01 (settings) — AccountSettings KASV | `grep -c "KeyboardAwareScrollView" src/screens/AccountSettingsScreen.tsx` | ≥ 1 | ⬜ |
| KBD-01 (chat) — 2 chat screens library-KAV | `grep -l "from 'react-native-keyboard-controller'" src/screens/Chat*Screen.tsx \| wc -l` | 2 | ⬜ |
...
| Field | Value |
|-------|-------|
| Walk start (UTC) | `{pending}` |
| Walk end (UTC) | `{pending}` |
| iOS build hash / Metro bundle fingerprint | `{pending}` |
| Android build hash / Metro bundle fingerprint | `{pending}` |
...
- [ ] Tier 1: 16/16 cells `Pass`
- [ ] Tier 2: 4/4 cells `Pass` (or accept-with-note for any `Fail`)
- [ ] L10 multiline: 6/6 cells `Pass`
- [ ] Android back-button: 2/2 cells `Pass`
- [ ] HomeScreen workaround comment decision executed (REMOVE or KEEP) with rationale
- [ ] All 12 phase-wide grep gates `Actual` match `Expected`
- [ ] `02-VALIDATION.md` frontmatter flipped: `status: complete`, `nyquist_compliant: true`
- [ ] `npx tsc --noEmit` exits 0
- [ ] Physical-device release builds succeed on both platforms
- [ ] `02-06-SUMMARY.md` written + committed (post-approval continuation agent)
