...
| 1-08-01 | 08 | 5 | ROLE-02 | T-1-07, T-1-08 | migrate-roles-m2.js syntax-clean; flips array; M1_ADMIN_EMAILS literal | smoke (grep + node --check) | `grep -q "M1_ADMIN_EMAILS = \['beckprograms@gmail.com'\]" src/scripts/migrate-roles-m2.js && node --check src/scripts/migrate-roles-m2.js` | ✓ after task | ✅ green (commit `3d2455c`) |
| 1-08-02 | 08 | 5 | ROLE-02 | T-1-07, T-1-08 | [BLOCKING] migration --verify=PASS against production Mongo; admin email userType=admin | manual ops | `npm run migrate:roles-m2 -- --verify` exits 0 + admin record `userType === 'admin'` (Atlas console spot-check; mongosh not installed locally) | n/a | ✅ green (run 2026-04-30 ~01:32 PT — see Manual-Only row) |
...
| Migration script run | ROLE-02 | 08 | One-shot Mongo write against production | `npm run migrate:roles-m2 -- --dry-run` → review counts → `npm run migrate:roles-m2` → `npm run migrate:roles-m2 -- --verify` exits 0; idempotent re-run all zeros — **EXECUTED 2026-04-30 ~01:32 PT against `bizdinkonush@ac-5hu8mt8-shard.mvry7rn.mongodb.net`. Dry-run: renter→user 12, owner→user 0, agent→user 0; admin promote `renter→admin`. Live run: 12 renters updated; admin promoted (path: `user→admin` after Step 1 flip ran first); post-migration $nin count = 0; ACCEPTANCE MET (ROLE-02). --verify: PASS (exit 0). Idempotent re-run: all flips 0; admin already admin (no-op). Atlas console spot-check confirmed `beckprograms@gmail.com.userType === 'admin'` (mongosh not installed locally — used Atlas web console).** |
