...
**Active workstreams:** M2 "Roles & Moderation". Phase 1 (backend role foundation + auth migration) closed 2026-04-30. Phase 2 (listing lifecycle status field absorption) closed 2026-05-01. Phase 3 (moderation queue + actions, MOD-10..MOD-18) closed 2026-05-02. **Phase 4 (archive lifecycle, ARCH-01..ARCH-05) closed 2026-05-03** with all 5 ARCH-IDs satisfied: backend `archivedReasonCode` + `archivedReasonNote` schema fields, `ModerationLog.action` enum extended with `archive`/`unarchive`/`hard-delete`, owner archive/unarchive routes (race-safe atomic `findOneAndUpdate`, D-13 two-condition unarchive gate, 4-way 403/404/409 disambiguation), mod/admin archive/restore routes (VALID_REJECT_CODES allowlist reuse, D-15 selective field clears with approve/reject preservation by omission, FIFO `submittedAt` re-stamp), DELETE /:id swapped to admin-only with full-snapshot audit row (Pitfall 2 inline-ownership block deleted), 106/106 backend test suite (was 86 baseline, +20 new across 5 describe blocks). RN client: `useRole` Action union extended with `archiveOwnListing`/`archiveAnyListing`/`hardDeleteListing`, 4 PropertyService methods (belt-and-suspenders `canFromUser` guards), `Property` type +4 archive audit fields, `ArchiveListingModal` forked from `RejectListingModal` (PATTERN A; chip-label + notePlaceholder keys reused), `RenterListingsScreen` Pitfall 6 atomic strip (delete affordances removed; `canSelfRestore` D-13 conditional gate hides Restore on mod-archived listings), `PropertyDetailsScreen` mod-action footer extension (Archive amber + Restore emerald + Hard Delete red) with sibling-mounted ArchiveListingModal + DeleteListingModal, 5 D-12 EN+RU locale value rewrites for re-moderation framing (key NAMES preserved per Pitfall 3). App.tsx D-19 net-zero target preserved (1191 LOC unchanged); manual physical-device smoke matrix walked APPROVED on iPhone 15 Pro Max (11/11 rows PASS; Moto G XT2513V deferred to Phase 6 REL-03). 0 critical / 3 warning / 5 info from code-review gate (advisory; all named invariants GREEN). Phase 5 (Admin Role Management UI) is the next plan-phase entry point.
...
---
*Last updated: 2026-05-03 — M2 Phase 4 (archive lifecycle ARCH-01..ARCH-05) closed; Phase 5 (Admin Role Management UI) unblocked*
