...
- [ ] **REL-05** Backend live + healthy on Railway. M3 changes deployed; `firebase-admin` STILL absent (carry-forward invariant); MongoDB Atlas + AWS IAM credentials confirmed unchanged from M2 baseline (HF-02 closed; AWS IAM cross-project residual remains documented PARTIAL — re-open condition unchanged). — **PRE-FLIGHT SUB-CRITERION SATISFIED 2026-05-08** by Plan 05-01: pre-flight runbook walked GREEN-WITH-DEVIATIONS across 14 sections; backend live at Railway SHA `5bf23fe` (`https://jaytap-services-production.up.railway.app`) with `health_check_status: 200`; firebase-admin still absent (verified via Phase 1 Plan 06-04 06-BACKEND-DEPLOY.md inheritance + Plan 05-01 backend env-var presence audit recording 7 keys including FIREBASE_PROJECT_ID); MongoDB Atlas password rotation (`mongo_revoked_at: 2026-04-29T00:00:00Z`) carries forward from M2 HF-02; AWS IAM remains PARTIAL (`aws_iam_path: jaytap_runtime_uses_new_user_only_old_user_retains_cross_project_policy`). 2 D-03 deviations operator-approved (Atlas snapshot SKIPPED + seed-locations-m3.js bugs patched mid-runbook). 2 M3 backlog items surfaced: `JayTap-orphan-data-cleanup` (drop pre-patch orphan collections) + `ATLAS-CRED-ROTATION` (defense-in-depth password rotation; non-blocking). Full evidence at `.planning/phases/05-hardening-manual-qa-release-v3/05-PREFLIGHT.md`. **GLOBAL CHECKBOX STAYS `[ ]`** — REL-05's "live + healthy" is a continuous condition that the dual-store submission (Plan 05-07) and the live device walks (Plan 05-05) verify against; the global flip happens at 05-07 close when the backend has been confirmed healthy throughout the full QA + submission window.
