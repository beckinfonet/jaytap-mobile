...
- [~] **ROLE-09** Axios response interceptor: `401 token-expired` → refresh idToken via ROLE-06 + retry once. `403 role-revoked` → call `AuthContext.refreshRole()` + retry once; if still 403, logout + show EN+RU "Your access changed. Please sign in again." toast. — **INTERCEPTOR COMPLETE 2026-04-30 (Plan 01-10 commit `13405e7` apiClient + `c7e0e1e` AuthContext); apiClient response interceptor switches on `error.response?.data?.code` per Plan 05/06 envelope contract: 'token-expired' → silent refresh + retry once, 'role-revoked' first-pass → refreshRole + retry once, 'role-revoked' POST-RETRY (final fallback) → `logoutHook?.()` + `toastHook?.('auth.accessChanged')`. Single-flight `refreshPromise` + `refreshRolePromise` mitigate refresh-token race (PITFALLS Pitfall 7); `_retry` guard prevents infinite loops; AuthContext's toast hook expands `auth.accessChanged` prefix to `${prefix}.title` + `${prefix}.body` for `Alert.alert`. Locale strings (`auth.accessChanged.title` "Your access changed" / «Ваш доступ изменился» + `auth.accessChanged.body` "Please sign in again." / «Пожалуйста, войдите снова.») land in Plan 01-12; until then `t()` returns the key string verbatim per `src/locales/index.ts:16` `?? key` fallback. Manual demote-twice smoke test deferred to Plan 11 service-migration window (apiClient never sees production traffic until services consume it).**
- [x] **ROLE-10** `AuthContext` exposes `refreshRole()` callable; bumps `user` reference on completion. Foreground role-refresh banner ("Your role changed — tap to reload" / «Ваша роль изменилась — нажмите для перезагрузки») shows when `userType` changes between two consecutive `refreshRole()` calls. — **CALLABLE COMPLETE 2026-04-30 (Plan 01-10 commit `c7e0e1e`); `AuthContext.refreshRole()` calls `apiClient.get<BackendProfile>('/auth/me')`, merges fresh BackendProfile into `user.backendProfile`, sets `isLoadingRole` true/false around the call, non-fatal on failure (cached profile preserved); exposed via `AuthContext.Provider value`. The foreground banner UI itself (RoleRefreshBanner mounted at App.tsx + the userType-change detection logic) lands in Plan 01-12.**
