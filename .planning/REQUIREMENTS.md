...
- [x] **REL-02** iOS `MARKETING_VERSION 3.0.0` + Android `versionName "3.0.0"`. Build numbers per **M1 D-02 lesson**: query `06-STORE-HISTORY.md` (re-run pattern from M2 Phase 6) for `next_ios_build_number = max(local, store) + 1` and `next_android_version_code = max(local, store) + 1` BEFORE the atomic version-bump commit. Avoids reactive bumps at archive time. — Plan 05-02 closed 2026-05-07: store-history captured at `.planning/phases/05-hardening-manual-qa-release-v3/05-STORE-HISTORY.md` via Path B web-UI user-relay; derived `next_ios_build_number=28` (max of local pbxproj=27, ASC TestFlight=26) + `next_android_version_code=31` (max of local build.gradle=30, Play Console Closed Alpha=29). Both platforms `local-ahead` by 1. M2 anchor discrepancy disclosed: `m2_anchor_observed_in_stores: false` — Play Console nothing at v2.0.0; TestFlight max=26 not 27. The actual atomic version bump (REL-01 + REL-02 source-file mutations) is owned by Plan 05-03; this REQ checkbox flips on the basis of the binding bump-targets being recorded for Plan 05-03 to consume.
