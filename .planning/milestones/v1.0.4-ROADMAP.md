...
phases_skipped: 1
phase_skipped_id: 7
phase_skipped_name: Alignment Pass
phase_skipped_reason: PROJECT.md Key Decisions row 129 — user confirmed nothing alignment-related stood out during Phase 1–6 QA
out_of_band_commits: [63f3b72]
shipped_ios_identifier: 1.0.4 build 22
shipped_android_identifier: 1.0.28 versionCode 28
xcode_toolchain: Xcode 26.4 (Build 17E192)
ios_submission_state: in_testflight
android_submission_state: submitted_processing
archived_from_sha: HEAD at archive time
---

# M1 "Polish + Hospitality" — v1.0.4 (Archived)

**Shipped:** 2026-04-28 to App Store Connect (in TestFlight, build 22) and Google Play Console (processing, versionCode 28).

This is the archived ROADMAP for the M1 milestone. Active roadmap is at `.planning/ROADMAP.md`. See `.planning/milestones/v1.0.4-REQUIREMENTS.md` for archived requirements.
...
- [x] **Phase 3: Role Gating Precursor** (2026-04-23) — Shipped `useRole()` + `can(action)` + `<Gated>` over hardcoded email allowlist; PropertyService canFromUser guard; CreateListingScreen + PropertyDetailsScreen + ProfileScreen migrations; D-14 4-part grep invariant CI gate; 15/15 jest GREEN; GATE-05 D-22 Path B accepted-risk for backend enforcement (closes via M2 ROLE-04)
...
- [x] **Phase 6: Hospitality Rendering** (2026-04-25) — `HospitalitySection` + `HospitalityCard` on Home/Favorites/Owner+RenterListings; tour-first `PropertyDetailsScreen` Hospitality branches (D-13/D-14/D-15/D-16/D-23 + HI-01 specs-row gap closure at `b1da946`); 12-item amenity taxonomy EN+RU; 80-cell QA matrix walked APPROVED on iPhone 15 Pro / iOS 26 + Moto G XT2513V / Android 16; verifier 6/6 must_haves PASS at `c534c0f`
...
**Plans**: 5 plans
  - [x] 05-01-validators-foundation-PLAN.md — Wave 1: pure validators.ts (validateByCategory + buildPayloadByCategory + Currency type + CURRENCY_OPTIONS + FIELD_ORDER_BY_CATEGORY) + Jest suite (~22 assertions) + FormBag.currency tightening + barrel exports — 2026-04-24
  - [x] 05-02-section-errors-threading-PLAN.md — Wave 2: thread errors through 5 sub-components (BasicInfo/Residential/Commercial/Hospitality/PriceSection) via `commonStyles.hint + colors.error` + migrate CURRENCY_OPTIONS to validators import — 2026-04-24
  - [x] 05-03-i18n-validation-keys-PLAN.md — Wave 2: 14 new EN+RU validation error keys in parity + createListing.publishListing for D-16 submit label — 2026-04-24
  - [x] 05-04-orchestrator-integration-PLAN.md — Wave 3: CreateListingScreen.tsx 10 edits (validator call + payload builder + errors state + scroll-to-first-error + D-11 contact Alert + D-16 status rule + onNavigateToAccountSettings prop); D-09 anchors preserved — 2026-04-24
  - [x] 05-05-nav-wiring-and-qa-matrix-PLAN.md — Wave 4: App.tsx onNavigateToAccountSettings wiring + Property `status?` type + 05-QA-MATRIX.md (54-cell physical-device scaffold) + 05-VERIFICATION.md (phase-exit regression bundle) — 2026-04-24
**UI hint**: yes
